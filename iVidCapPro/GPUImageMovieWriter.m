...
// eOe start...
NSString *const kGPUImageSwizzleAndGammaFragmentShaderString = SHADER_STRING
(
 varying highp vec2 textureCoordinate;
 ...
 void main()
 {
     lowp vec4 textureColor = texture2D(inputImageTexture, textureCoordinate).bgra;
     
     gl_FragColor = vec4(pow(textureColor.bgr, vec3(gamma)), textureColor.a);
 }
 );

NSString *const kGPUImageMovieGammaFragmentShaderString = SHADER_STRING
(
 varying highp vec2 textureCoordinate;
...
{
    return [self initWithMovieURL:newMovieURL size:newSize fileType:AVFileTypeMPEG4 gammaEnable:false outputSettings:nil];
}
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType gammaEnable:(bool)allowGammaAdjust outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    // eOe - This input rotation setting is apparently unused.
    //inputRotation = kGPUImageFlipVertical;
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([GPUImageOpenGLESContext supportsFastTextureUpload])
        {
            ...
            if (!allowGammaAdjust)
                ...
                colorSwizzlingProgram = [[GPUImageOpenGLESContext sharedImageProcessingOpenGLESContext] programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kGPUImageMovieGammaFragmentShaderString];
            // eOe end
        }
        else
        {
            ...
            if (!allowGammaAdjust)
                ...
                colorSwizzlingProgram = [[GPUImageOpenGLESContext sharedImageProcessingOpenGLESContext] programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kGPUImageSwizzleAndGammaFragmentShaderString];
            // eOe end
        }
        ...
        // eOe start...
        gammaUniform = [colorSwizzlingProgram uniformIndex:@"gamma"];
        self.gamma = 1.0;
        // eOe end
...
- (void) recordingCompleteHandler {
    //NSLog(@"recordingCompleteHandler called... recording status: %d \n", assetWriter.status);
}
// eOe end
...
- (void)finishRecording
{
    ...
    // eOe To-Do: Consider changing this to run on a different queue.
    //   Maybe it will prevent stutter?  Stutter should only occur on
    //   ios5, since on ios6 we finish with completion handler, which
    //   already runs on a different queue.
    runOnMainQueueWithoutDeadlocking(^{
        ...
        if( [ [[UIDevice currentDevice] systemVersion] compare: @"6.0" options: NSNumericSearch ] != NSOrderedAscending ) {
            // Device is running ios6 or newer, so we can use this method.
            //NSLog(@"\nAbout to call assetWriter finishWritingWithCompletionHandler...\n");
            [assetWriter finishWritingWithCompletionHandler:^{
                [self recordingCompleteHandler];
            }];
        } else {
            // For older versions of ios we must use this method.
            //NSLog(@"\nAbout to call assetWriter finishWriting...\n");
...
- (void)processAudioBuffer:(CMSampleBufferRef)audioBuffer;
{
    ...
    //NSLog(@"processAudioBuffer: called... isRecording=%d  _hasAudioTrack=%d\n", isRecording, _hasAudioTrack);
    if (!isRecording)
    {
        return;
    }
...
- (void)renderAtInternalSize;
{
    ...
    //eOe
    // Rearrange vertices to cause the video to be rendered right side up.
    static const GLfloat squareVertices[] = {
        -1.0f, 1.0f,
         1.0f, 1.0f,
        -1.0f,  -1.0f,
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    //NSLog(@"newFrameReadyAtTime called...\n");

    if ([GPUImageOpenGLESContext supportsFastTextureUpload])
    {
        pixel_buffer = renderTarget;
        CVPixelBufferLockBaseAddress(pixel_buffer, 0);
        //NSLog(@"newFrameReadyAtTime: fast texture\n");
    }
    else
    {
        //NSLog(@"newFrameReadyAtTime: slow texture\n");
    ...
    if(![assetWriterPixelBufferInput appendPixelBuffer:pixel_buffer withPresentationTime:frameTime]) 
    {
        ...
        if (failureBlock)
        {
            NSString* localizedDescription = @"appendPixelBuffer_failed";
            NSDictionary *errorValues = [NSDictionary dictionaryWithObjectsAndKeys:
                                         localizedDescription, NSLocalizedDescriptionKey,
                                         nil];
            NSError* error = [NSError errorWithDomain:@"GPUImage_MovieWriter" code:1 userInfo:errorValues];
            failureBlock(error);
        }
    } 
    else 
...
{
    return assetWriterVideoInput.readyForMoreMediaData;
}
...
- (void)setGamma:(CGFloat)newValue;
{
    ...
    [self setFloat:_gamma forUniform:gammaUniform program:colorSwizzlingProgram];
}
// eOe end
