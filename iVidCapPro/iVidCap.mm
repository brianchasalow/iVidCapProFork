...
- (int) endRecordingSession: (VideoDisposition) action AddAudioFile1:(NSString*) audioFile1 AddAudioFile2:(NSString*) audioFile2 {
    ...
    [opacityFilter removeTarget:movieWriter];
    ...
    if (action == Discard_Video) {
        ...
        if (captureAudio == Audio || captureAudio == Audio_Plus_Mic || audioFile1 != nil || audioFile2 != nil) {
            // We have audio.  Mix the audio and video.
            [self createVideoWithAudioUserFile1: audioFile1 userFile2: audioFile2];
            
        } else {
            // There's no audio. Create the final video file without mixing.
            
            // Let developer know when they specified an audio file but it's not found.
            // To-do:  Move this above the if statement?
            if (showDebug && audioFile1 != nil && ![self fileExists:audioFile1])
                NSLog(@"iVidCapPro - endRecordingSession: user specified audio file 1 does not exist.  It is ignored.  %@", audioFile1);
            if (showDebug && audioFile2 != nil && ![self fileExists:audioFile2])
                NSLog(@"iVidCapPro - endRecordingSession: user specified audio file 2 does not exist.  It is ignored.  %@", audioFile2);
...
- (void) createVideoWithAudioUserFile1: (NSString*) userAudioFile1 userFile2: (NSString*) userAudioFile2 {
    ...
    if (userAudioFile1 != nil) {
        ...
        // Make sure the specified file exists.  If not, it will be ignored.
        if ([self fileExists:userAudioFile1])
            userAudioFileURL1 = [NSURL fileURLWithPath:userAudioFile1];
        else if (showDebug)
            NSLog(@"iVidCapPro - createVideoWithAudioUserFile: user specified audio file 1 does not exist.  It is ignored.  %@", userAudioFile1);
    }
    ...
    if (userAudioFile2 != nil) {
        ...
        if ([self fileExists:userAudioFile2])
            ...
            NSLog(@"iVidCapPro - createVideoWithAudioUserFile: user specified audio file 2 does not exist.  It is ignored.  %@", userAudioFile2);
    }
...
- (void) movieWriterErrorHandler: (NSError *) error {
    ...
    if (showDebug)
        NSLog(@"iVidCapPro - movieWriterErrorHandler: error code=%d description=%@", [error code], [error localizedDescription] );
...
- (void) copyToPhotoAlbumCompleteFromVideo: (NSString *) videoPath
    ...
    if (strcmp(iVidCapGameObject, "") != 0)
    {
        if ([error code] == 0) {
            ...
            if (showDebug)
                NSLog(@"iVidCapPro - copyToPhotoAlbumCompleteFromVideo: Failed! error code=%d description=%@\n", [error code], [error localizedDescription]);
        }
    }
...
- (void)mixAudioFiles:(NSURL*)capturedAudioFileURL UserAudioFile1:(NSURL*) userAudioFileURL1 UserAudioFile2:(NSURL*) userAudioFileURL2 VideoFile: (NSURL*) videoFileURL ToOutput: (NSURL*) outputFileURL {
    ...
    if (capturedAudioFileURL != nil) {
        numAudioTracks++;
        [self LoadCapturedAudioAsset: capturedAudioFileURL];
    }
    if (userAudioFileURL1 != nil) {
        numAudioTracks++;
        [self LoadUserAudioAsset1: userAudioFileURL1];
    }
    if (userAudioFileURL2 != nil) {
        numAudioTracks++;
        [self LoadUserAudioAsset2: userAudioFileURL2];
    }
    ...
    if (numAudioTracks > 1) {
        ...
        if (capturedAudioFileURL != nil)
            [self LoadMixedAudioAsset: capturedAudioFileURL];
        else if (userAudioFileURL1 != nil)
            [self LoadMixedAudioAsset: userAudioFileURL1];
        else
            [self LoadMixedAudioAsset: userAudioFileURL2];
        [self LoadVideoAsset: videoFileURL];
...
{
    if (showDebug)
        NSLog(@"iVidCapPro - audioMixDidFinish called... outputURL=%@ status=%d", session.outputURL, session.status);
    ...
    if (session.status == AVAssetExportSessionStatusCompleted) {
        ...
        [self LoadMixedAudioAsset: session.outputURL];
        NSURL* videoFileURL = [self getDocumentsFileURL:tempVideoFileName];
        NSURL* outputFileURL = [self getDocumentsFileURL:finalVideoFileName];
        [self LoadVideoAsset: videoFileURL];
        [self ComposeTracksToURL:outputFileURL];
...
- (void)exportDidFinish:(AVAssetExportSession*)session
{
    NSString *filePath;
    if (showDebug)
        NSLog(@"iVidCapPro - exportDidFinish called... outputURL=%@ status=%d", session.outputURL, session.status);
